...
const customPrefixTable = pgTable("users_custom_prefix", {
  id: cuid2("id").setPrefix("usr_").defaultRandom().primaryKey(),
  name: text("name"),
});

describe("pgCuid2 custom prefix", () => {
...
const customPrefixAndLengthTable = pgTable("users_custom_prefix_length", {
  id: cuid2("id").setPrefix("post_").setLength(16).defaultRandom().primaryKey(),
  name: text("name"),
});

describe("pgCuid2 custom prefix and length", () => {
